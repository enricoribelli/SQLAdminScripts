...
*
* Modified 2020-02-11 By Lutfi Uzun
*
* Added check constraints and drop alter for it
*
*********************************************************************************/
...
IF (@DatabaseCollation <> 'Latin1_General_CI_AS')
BEGIN 
	-- Generate DROP CONSTRAINT statement
	INSERT INTO #tempscriptstore (ScriptType, script)
	VALUES  ('DbCollationUpdate', N'ALTER DATABASE ' + QUOTENAME(DB_NAME()) + ' SET SINGLE_USER WITH ROLLBACK IMMEDIATE;'),
			('DbCollationUpdate', N'GO'),
			('DbCollationUpdate', N'ALTER DATABASE ' + QUOTENAME(DB_NAME()) + ' COLLATE Latin1_General_CI_AS;'),
			('DbCollationUpdate', N'GO'),
			('DbCollationUpdate', N'ALTER DATABASE ' + QUOTENAME(DB_NAME()) + ' SET MULTI_USER WITH ROLLBACK IMMEDIATE;'),
			('DbCollationUpdate', N'GO');
END

SET @DatabaseCollation = 'Latin1_General_CI_AS';
        ...
        BEGIN;
            ...
            SELECT      fk.object_id                                      AS constid,
                        ...
                        CASE WHEN fk.delete_referential_action <> 0 -- No action
                                 THEN N' ON DELETE ' + CASE 
															WHEN fk.delete_referential_action_desc='SET_NULL' THEN  REPLACE(fk.delete_referential_action_desc, 'SET_NULL','SET NULL') 
															ELSE fk.delete_referential_action_desc
													   END
                             ELSE N''
                        END                                               AS delete_cascade,
                        CASE WHEN fk.update_referential_action <> 0 -- No action
                                 THEN N' ON UPDATE ' + CASE 
															WHEN fk.update_referential_action_desc='SET_NULL' THEN  REPLACE(fk.update_referential_action_desc, 'SET_NULL','SET NULL') 
...
*/
	...
	--	)
	--	INSERT INTO #tempscriptstore (ScriptType, script)
	--	SELECT 'DropCheck', N'ALTER TABLE ' + QUOTENAME(@SchemaName) + N'.' + QUOTENAME(@TableName) + N' DROP CONSTRAINT ' + QUOTENAME(name) + ' '
	--	FROM cte_contraints
	--	WHERE cte_contraints.object_id = @object_id 
	--			AND collation_name <> @DatabaseCollation;
	...
	--	)
	--	INSERT INTO #tempscriptstore (ScriptType,script)
	--	SELECT 'CreateCheck', N'ALTER TABLE ' + QUOTENAME(@SchemaName) + N'.' + QUOTENAME(@TableName) + N' WITH CHECK ADD CONSTRAINT ' + QUOTENAME(cte.name) + ' CHECK ' + cte.definition
	--	FROM cte_contraints cte
	--	WHERE cte.object_id = @object_id 
	--		AND cte.collation_name <> @DatabaseCollation
	--	GROUP BY cte.name, cte.definition;
...
INSERT INTO #tempscriptstore (ScriptType, script)
SELECT 'DropCheck', N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(schema_id)) + N'.' + QUOTENAME(objectname) + N' DROP CONSTRAINT ' + QUOTENAME(name) + ' '
FROM cte_contraints
...
INSERT INTO #tempscriptstore (ScriptType,script)
SELECT 'CreateCheck', N'ALTER TABLE ' + QUOTENAME(SCHEMA_NAME(cte.schema_id)) + N'.' + QUOTENAME(cte.objectname) + N' WITH CHECK ADD CONSTRAINT ' + QUOTENAME(cte.name)+ ' CHECK ' + cte.definition
FROM cte_contraints cte
...
WITH ScriptInput
  AS (SELECT    script,
                CASE WHEN ScriptType = 'DbCollationUpdate' 
						THEN  1
      ...
      UNION ALL
      SELECT    CASE WHEN n.num=8 THEN 'GO -- End of dropping objects, before changing collations'
					 WHEN n.num=10 THEN 'GO -- End of changing collations, before re-creating objects'
					 WHEN n.num=17 THEN 'GO -- End of creating objects, before checking and disabling foreign keys and indexes'
					 WHEN n.num=21 THEN 'GO -- End of script'
				END,
                n.num
      FROM      (VALUES (8),    -- After dropping objects, before changing collations
                        (10),    -- After chaning collations, before re-creating objects
                        (17),   -- After creating objects, before checking and disabling foreign keys and indexes
                        (21)    -- End of script
      ) AS n (num) )
